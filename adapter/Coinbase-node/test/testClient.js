...
describe('Client', function() {
  ...
  describe('client constructor', function() {
    it('should return client', function() {
      var client = new Client({'apiKey': 'LQIxV1ne9l9w9fWj', 'apiSecret': '72dPDid8BZ3ZTYA8hQKeDdvUnBt4K5Wp', 'baseApiUri': na.TEST_BASE_URI, 'timeout': 30000});
      assert(client);
      assert.equal(client.apiKey, 'LQIxV1ne9l9w9fWj');
      assert.equal(client.apiSecret, '72dPDid8BZ3ZTYA8hQKeDdvUnBt4K5Wp');
      assert.equal(client.timeout, 30000);
    });
    it('should has default option values', function() {
        var client = new Client({'apiKey': 'LQIxV1ne9l9w9fWj', 'apiSecret': '72dPDid8BZ3ZTYA8hQKeDdvUnBt4K5Wp'});
        assert(client);
    ...
    it('should require constructor new call', function(){
      var cl1 = Client({'apiKey': 'LQIxV1ne9l9w9fWj', 'apiSecret': '72dPDid8BZ3ZTYA8hQKeDdvUnBt4K5Wp', 'baseApiUri': na.TEST_BASE_URI});
    ...
    it('should throw if both the api key + secret and the access token is used', function() {
      try {
        Client({ 'accessToken': 'mytoken', 'apiKey': 'LQIxV1ne9l9w9fWj', 'apiSecret': '72dPDid8BZ3ZTYA8hQKeDdvUnBt4K5Wp' })
        throw new Error('failed');
      } catch (e) {
        assert.equal(e.message, 'you must either provide an "accessToken" or the "apiKey" & "apiSecret" parameters')
      }
