...
export default class ContractSearch extends React.Component {
	...
	handleLoad(event) {
        ...
        this.loadXBORDERContract(this.state.currentContract);
	}
	...
	*/
	...
	//Loader function to retrieve the data from the contract
	async loadXBORDERFactoryContract (address,isBuyer) {
		try{
			XBORDER = new web3.eth.Contract(abiXBORDER, this.state.currentContract);
			if (isBuyer){
				await XBORDERFactory.methods.getXBORDERAddressesBuyer().call({from:address}).then(function (res) { console.log(res); this.setState({ contracts  : res })}.bind(this));
			}else{
				await XBORDERFactory.methods.getXBORDERAddressesSeller().call({from:address}).then(function (res) {  console.log(res); this.setState({ contracts  : res })}.bind(this));
			}
		...
		return(  
		  ...
		  <div>
				<Grid container  justify = "center" spacing={1} alignItems="flex-end">
					<Grid item>
					<br/>
					<Fab variant="extended" padding={5} onClick={this.loadXBORDERFactoryContract} aria-label="like">
					<NavigationIcon />
					Deposit LINK
					</Fab>
					</Grid>
		  ...
		  {/* //loadXBORDERFactoryContract (address,isBuyer) */}
